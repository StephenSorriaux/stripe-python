...
class APIRequestor(object):
    ...
    async def async_request(self, method, url, params=None, headers=None):
        rbody, rcode, rheaders, my_api_key = await self.async_request_raw(
            method.lower(), url, params, headers
        )
        resp = self.interpret_response(rbody, rcode, rheaders)
        return resp, my_api_key
    ...
    async def async_request_raw(self, method, url, params=None, supplied_headers=None):
        """
        Mechanism for issuing an API call
        """
        ...
        util.log_info("Async request to Stripe api", method=method, path=abs_url)
        util.log_debug(
            "Post details",
            post_data=encoded_params,
            api_version=self.api_version,
        ...
        rbody, rcode, rheaders = await self._client.async_request_with_retries(
            method, abs_url, headers, post_data
        )

        util.log_info("Async Stripe API response", path=abs_url, response_code=rcode)
        util.log_debug("API response body", body=rbody)
